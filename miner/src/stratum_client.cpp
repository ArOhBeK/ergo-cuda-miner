...
void StratumClient::submit_share(const std::string& job_id, uint64_t nonce, const Job& job) {
    ...
    std::cout << "[DEBUG] Job ID: " << job_id << std::endl;
    std::cout << "[DEBUG] Nonce: " << nonce << std::endl;
    std::cout << "[DEBUG] Nonce LE: " << bytes_to_hex(nonce_le) << std::endl;
    std::cout << "[DEBUG] Header: " << bytes_to_hex(job.header_hash) << std::endl;
    std::cout << "[DEBUG] Hash: ";
    for (int i = 0; i < 32; ++i) std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)hash_output[i];
    std::cout << std::endl;
    std::cout << "[DEBUG] Target: " << bytes_to_hex(job.target_difficulty) << std::endl;

    if (memcmp(hash_output, job.target_difficulty.data(), 32) >= 0) {
        std::cout << "[-] Rejected by target difficulty" << std::endl;
        ++rejected_shares;
        return;
    }
...
std::string StratumClient::get_gpu_stats() {
    FILE* pipe = popen("/usr/bin/nvidia-smi --query-gpu=temperature.gpu,power.draw,utilization.gpu,clocks.gr --format=csv,noheader,nounits", "r");

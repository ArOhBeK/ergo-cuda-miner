...
__global__ void miner_kernel(const uint8_t* d_table, const uint8_t* d_header, const uint8_t* d_target, size_t table_size, uint64_t* d_nonce_out, int* d_found) {
    ...
    uint8_t input[72];
    memcpy(input, d_header, 32);
    *((uint64_t*)(input + 32)) = nonce;
    for (int i = 0; i < 32; ++i)
        input[40 + i] = d_table[i];

    uint8_t hash[32];
    blake2b_hash(input, 72, hash);

    for (int i = 0; i < 32; ++i) {
        if (hash[i] < d_target[i]) break;
        if (hash[i] > d_target[i]) return;
    }

    if (atomicCAS(d_found, 0, 1) == 0) *d_nonce_out = nonce;
}

uint64_t launch_gpu_miner(const std::vector<std::array<uint8_t, 32>>& dag, const std::vector<uint8_t>& header, const std::vector<uint8_t>& target) {
